...
# note, these methods must be loaded from modules to workers (living on MFWIs#newmpi), but this is what they are

# function foo9mpi(x, tsk, a; b)
#     MPI.Init()
#     comm = MPI.COMM_WORLD
...
#     MPI.Init()
#     MPI.Barrier(MPI.COMM_WORLD)
#     # localresult = fetch(_localresult)
#     localresult = (MPI.Comm_rank(MPI.COMM_WORLD) == 0 ? fetch(_localresult) : nothing)
#     MPI.Barrier(MPI.COMM_WORLD)
#     local2 = MPI.bcast(localresult, 0, MPI.COMM_WORLD)
#     MPI.Barrier(MPI.COMM_WORLD)
...
@testset "schedulers with mpi epmap reduce test" begin
    N = 100 
